...
def load_mnist(num_folds=5):
    # load and split the dataset into train and test sets
    (x_train_set, y_train_set), test_set = keras.datasets.mnist.load_data(path="/home/hpc/iwi5/iwi5084h/coincidence_detection_in_snns/training/mnist.npz")
    train_fold = []
    val_fold = []
...
with tf.device('GPU:0'):
    ...
    for decay_distribution in decay_initializers:
        ...
        for t_dec in decays:
            ...
            train_fold, val_fold, test_set = load_mnist(num_folds)
            ...
            for fold in range(0, num_folds):
                ...
                model_name = f"mnist_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}_{decay_distribution}_decay{t_dec}ms_fold{fold}"
