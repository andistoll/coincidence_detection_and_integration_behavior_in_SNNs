...
# !!!!!! this is not the normal mnist dataset but a modified version !!!!!!!
from training.brightness_offset_mnist import load_mnist_with_brightness_offset
...
with tf.device('GPU:0'):
    ...
    for decay_distribution in decay_initializers:
        ...
        for t_dec in decays:
            ...
            train_fold, val_fold, test_set = load_mnist_with_brightness_offset(num_folds)
            ...
            for fold in range(0, num_folds):
                ...
                model_name = f"mnist_with_brightness_offset_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}_{decay_distribution}_decay{t_dec}ms_fold{fold}"
                model.save(f"./models/dt{dt}_untrained/{experiment}/" + model_name)


                # fit the model
                output_path = f"mnist_with_brightness_offset_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}"
