...
from brightness_offset_lettermnist import load_letter_mnist_with_negative_brightness_offset


with tf.device('GPU:0'):
    print("EMNIST/Letters adj (negative)")
    decay_initializers = ['constant']
    decays = [15, 30, 45, 60, 75] 
    #decays = [90, 180, 285, 390, 480]  # [ms]
    ...
    experiment = "no_bias_gaussian_weights_128_adj_neg_offset" # TODO: make sure this folder exists in dt{dt} and dt{dt}_untrained
    ...
    for decay_distribution in decay_initializers:
        ...
        for t_dec in decays:
            ...
            train_fold, val_fold, test_set = load_letter_mnist_with_negative_brightness_offset(num_folds)
            ...
            for fold in range(0, num_folds):
                ...
                model_name = f"letter_mnist_with_brightness_offset_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}_{decay_distribution}_decay{t_dec}ms_fold{fold}"
                model.save(f"./models/dt{dt}_untrained/{experiment}/" + model_name)


                # fit the model
                output_path = f"letter_mnist_with_brightness_offset_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}"
