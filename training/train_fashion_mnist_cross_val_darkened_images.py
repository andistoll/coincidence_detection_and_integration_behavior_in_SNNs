...
from brightness_offset_fashionmnist import load_fashion_mnist_with_brightness_offset


with tf.device('GPU:0'):
    print("Fashion-MNIST adj")
    ...
    for decay_distribution in decay_initializers:
        ...
        for t_dec in decays:
            ...
            train_fold, val_fold, test_set = load_fashion_mnist_with_brightness_offset(num_folds)
            ...
            for fold in range(0, num_folds):
                ...
                model_name = f"fashion_mnist_with_brightness_offset_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}_{decay_distribution}_decay{t_dec}ms_fold{fold}"
                model.save(f"./models/dt{dt}_untrained/{experiment}/" + model_name)


                # fit the model
                output_path = f"fashion_mnist_with_brightness_offset_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}"
