...
def load_fashion_mnist(num_folds=5):
    ...
    test_set = load_local_fashion_mnist(data_path, kind='t10k')
...
with tf.device('GPU:0'):
    ...
    for decay_distribution in decay_initializers:
        ...
        for t_dec in decays:
            ...
            train_fold, val_fold, test_set = load_fashion_mnist(num_folds)
            ...
            for fold in range(0, num_folds):
                ...
                model_name = f"fashion_mnist_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}_{decay_distribution}_decay{t_dec}ms_fold{fold}"
                model.save(f"./models/dt{dt}_untrained/{experiment}/" + model_name)


                # fit the model
                output_path = f"fashion_mnist_time{time_steps}_dt{dt}_poisson_hidden{num_lif_units}"
